...
def StackReassort(dir1, file_lst, file_name):
    ...
    for i in range(inputs.shape[0]):
        ...
        length = len(inputs[i])
        #    for j in range(1280 - length):
        #        inputs[i] = np.append(inputs[i], np.zeros(64), axis = 0)
        tmp_inputs = np.vstack((inputs[i], np.zeros((Maxlen - length, 64))))
        inputs2.append(tmp_inputs)
    print(len(inputs2[0]))

    np.save('../Res/npy_reassort/array_codon_freq_' + file_name[:-1] + '.npy', inputs2, allow_pickle=True)
