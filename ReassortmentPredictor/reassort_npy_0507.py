...
def StackReassort(dir1, file_lst, file_name):
    ...
    for i in range(len(file_lst)):
        ...
        inputs = np.load('../Res/model_evaluate/lucaOne/new_AIV_all_8_' + file_name + cds[i] + '_gene_emb_dnabert2_array_0503_ordered.npy',
                         allow_pickle=True)
        tmp_inputs = []
    ...
    for i in range(inputs.shape[0]):
        ...
        length = len(inputs[i])
        tmp_inputs = np.vstack((inputs[i], np.zeros((Maxlen - length, 2560))))
        inputs2.append(tmp_inputs)
    print(len(inputs2[0]))

    np.save('../Res/npy_reassort/array_codon_freq_' + file_name[:-1] + '_gene_emb_dnabert2_array_0503_ordered_2816.npy', inputs2, allow_pickle=True)
