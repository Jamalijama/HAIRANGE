...
for i in range(len(cds)):
    ...
    freq_path = '../Res/model_evaluate/ESM2_05/new_AIV_all_8_' + file_name + '_protein_emb_esm2_array_0503_ordered.npy'
    ...
    for mdl_batch_size in batch_size_lst:
        for lr in lr_lst:
            ...
            for j in range(n_splits):
                ...
                if valid_acc > best_valid_acc:
                    ...
                    torch.save(model.state_dict(),
                                '../Res/pt/resnet_classification_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.pt')

                xx += 1

            PrintTrain(x, train_loss_lst, valid_acc_lst, '../Res/fig/train_' + file_name + '_ESM2_05_' + str(mdl_batch_size) +
                        '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.png')
            PrintROCPRAndCM(trues, preds, probs, pos_label,
                            '../Res/fig/cm_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34',
                            '../Res/fig/roc_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.png',
                            '../Res/fig/pr_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.png',
                            '../Res/train_result_data/roc_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv',
                            '../Res/train_result_data/pr_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv')
            df1 = pd.DataFrame()
            df1['train_loss'] = train_loss_lst
            df1['train_acc'] = train_acc_lst
            df1['valid_acc'] = valid_acc_lst
            df1.to_csv('../Res/train_result_data/train_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv', index=False)
            
            df1 = pd.DataFrame()
            df1['valid_f1'] = valid_f1_lst
            df1.to_csv('../Res/train_result_data/valid_f1_' + file_name + '_ESM2_05_' + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv', index=False)
            end_time = time()
            ...
            with open('../Res/times/ESM2_05_train_result_' + str(N_EPOCHS * n_splits) + '.txt', 'a') as f:
                print(cds[i], file=f)
