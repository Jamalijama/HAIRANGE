...
for i in range(0, 1):
    ...
    for mdl_batch_size in batch_size_lst:
        for lr in lr_lst:
            ...
            file_name += '_ESM2_05_7_' + str(N_EPOCHS * n_splits)
            ...
            for j in range(n_splits):
                ...
                if valid_acc > best_valid_acc:
                    ...
                    torch.save(model.state_dict(),
                                '../Res/pt/resnet_classification_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.pt')
            ...
            PrintROCPRAndCM(trues, preds, probs, pos_label,
                            '../Res/fig/cm_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34',
                            '../Res/fig/roc_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.png',
                            '../Res/fig/pr_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.png',
                            '../Res/train_result_data/roc_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv',
                            '../Res/train_result_data/pr_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv')
            df1 = pd.DataFrame()
            df1['train_loss'] = train_loss_lst
            df1['train_acc'] = train_acc_lst
            df1['valid_acc'] = valid_acc_lst
            df1.to_csv('../Res/train_result_data/train_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv', index=False)
            
            df1 = pd.DataFrame()
            df1['valid_f1'] = valid_f1_lst
            df1.to_csv('../Res/train_result_data/valid_f1_' + file_name + str(mdl_batch_size) + '+' + str(lr) + '+' + str(N_EPOCHS * n_splits) + '_resnet34.csv', index=False)
            end_time = time()
