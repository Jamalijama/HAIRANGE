...
NOTE: ALL FILES UNDER `/Res/res1` HAVE BEEN COPPRESSED INTO THE FILE, `/Res/res1.zip`, PLEASE UNDEPRESSED THE ZIP FILE BEFORE UTILIZING.
...
1.  Run `reassort_npy.py` to concat encoding vectors of important codons of four fragments together according to the results of ablation experiments.
    We filled with zero vectors to satisfy the required shape for training the model.
    We concat sequences before 2020 and given reassort sequences here, you can change the `file_name` to concat other sequences.
...
7.  Run `reassort_single.py` to concat encoding vectors of important codons without filling zero vectors of four fragments respectively and generate four output files.
...
9.  Run `simulate_seq.py` to generate simulated reassortment sequences in encoding matrix format with two model sequences of human H3N2 and human H1N1 subtype.
    We simulate genomes with H1N1 subtype here, you can change the model name to simulate H3N2 genomes.
